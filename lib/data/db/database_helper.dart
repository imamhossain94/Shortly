import 'package:sqflite/sqflite.dart';
import 'package:path/path.dart';
import '../models/url_data.dart';

class DatabaseHelper {
  static final DatabaseHelper instance = DatabaseHelper._init();
  static Database? _database;

  DatabaseHelper._init();

  Future<Database> get database async {
    if (_database != null) return _database!;
    _database = await _initDB('shortly.db');
    return _database!;
  }

  Future<Database> _initDB(String filePath) async {
    final dbPath = await getDatabasesPath();
    final path = join(dbPath, filePath);

    return await openDatabase(path, version: 1, onCreate: _createDB);
  }

  Future _createDB(Database db, int version) async {
    const idType = 'INTEGER PRIMARY KEY AUTOINCREMENT';
    const textType = 'TEXT';
    const boolType = 'INTEGER';
    const integerType = 'INTEGER';

    await db.execute('''
CREATE TABLE url_data ( 
  id $idType, 
  provider $textType,
  originalUrl $textType,
  shortenedUrl $textType,
  expandedUrl $textType,
  success $boolType,
  timestamp $integerType
  )
''');
  }

  Future<int> insert(UrlData urlData) async {
    final db = await instance.database;
    return await db.insert('url_data', urlData.toMap());
  }

  Future<List<UrlData>> getAllUrls() async {
    try {
      final db = await instance.database;
      const orderBy = 'timestamp DESC';
      final result = await db.query('url_data', orderBy: orderBy);
      return result.map((json) => UrlData.fromMap(json)).toList();
    } catch (e) {
      print('Error getting all urls: $e');
      return [];
    }
  }

  Future<int> delete(int id) async {
    final db = await instance.database;
    return await db.delete('url_data', where: 'id = ?', whereArgs: [id]);
  }

  Future<void> clearAll() async {
    final db = await instance.database;
    await db.delete('url_data');
  }

  Future<void> seedData([Database? db]) async {
    final database = db ?? await instance.database;
    final List<UrlData> sampleUrls = [
      UrlData(
        provider: 'Cleanuri',
        originalUrl: 'https://example.com/article/how-to-start-coding',
        shortenedUrl: 'https://cleanuri.com/a1b2c3',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 100000,
      ),
      UrlData(
        provider: 'Shorte.st',
        originalUrl: 'https://mysite.com/portfolio/creative-works',
        shortenedUrl: 'http://sh.st/p4q5r6',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 200000,
      ),
      UrlData(
        provider: 'Bitly',
        originalUrl: 'https://awesome-designs.net/ui-ux-trends-2024',
        shortenedUrl: 'https://bit.ly/d7e8f9',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 300000,
      ),
      UrlData(
        provider: 'TinyURL',
        originalUrl: 'https://travel-explorer.org/top-10-destinations',
        shortenedUrl: 'https://tinyurl.com/t1u2v3',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 400000,
      ),
      UrlData(
        provider: 'Is.gd',
        originalUrl: 'https://coding-academy.io/learn-flutter-fast',
        shortenedUrl: 'https://is.gd/f4g5h6',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 500000,
      ),
      UrlData(
        provider: 'Cutt.ly',
        originalUrl: 'https://business-insights.com/market-analysis-2024',
        shortenedUrl: 'https://cutt.ly/m7n8o9',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 600000,
      ),
      UrlData(
        provider: 'V.gd',
        originalUrl: 'https://healthy-living.net/daily-workout-routine',
        shortenedUrl: 'https://v.gd/w1x2y3',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 700000,
      ),
      UrlData(
        provider: 'T.ly',
        originalUrl: 'https://photography-mastery.com/golden-hour-tips',
        shortenedUrl: 'https://t.ly/p4s5t6',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 800000,
      ),
      UrlData(
        provider: 'S.id',
        originalUrl: 'https://music-discovery.fm/indie-playlist-weekly',
        shortenedUrl: 'https://s.id/m7l8k9',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 900000,
      ),
      UrlData(
        provider: 'Buff.ly',
        originalUrl: 'https://tech-reviews.io/latest-gadget-leaks',
        shortenedUrl: 'https://buff.ly/g1h2i3',
        success: true,
        timestamp: DateTime.now().millisecondsSinceEpoch - 1000000,
      ),
    ];

    for (final url in sampleUrls) {
      await database.insert('url_data', url.toMap());
    }
  }
}
